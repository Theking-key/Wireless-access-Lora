...
void SX1276SetPreambleLength(uint16_t preambleLen)
{
	SX1276.Settings.LoRa.PreambleLen = preambleLen;
...
//    DelayMs( 6 );
	...
	HAL_Delay(1);
	HAL_GPIO_WritePin(VRADIO_RESET_GPIO_Port, VRADIO_RESET_Pin, GPIO_PIN_SET);
	//HAL_Delay(6);
}
...
static HAL_StatusTypeDef returnStatus;
...
void SX1276WriteBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
	...
	addr = addr | 0x80;
	returnStatus = HAL_SPI_Transmit(SX1276.Spi, &addr, 1, 300);
	returnStatus = HAL_SPI_Transmit(SX1276.Spi, buffer, size, 500);
	

	HAL_GPIO_WritePin(VRADIO_NSS_GPIO_Port, VRADIO_NSS_Pin, GPIO_PIN_SET);
}
...
void SX1276ReadBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
	...
	addr = addr & 0x7F;
	returnStatus = HAL_SPI_Transmit(SX1276.Spi, &addr, 1, 300);
	returnStatus = HAL_SPI_Receive(SX1276.Spi, buffer, size, 500);
            ...
            case MODEM_LORA:
                {
					...
					ManageFlag = SX1276Read( REG_LR_MODEMSTAT );
					
//					if((ManageFlag & 0x01) == 0x01)//检测到lora前导码
//						HAL_UART_Transmit(&huart2, (uint8_t*)"Detect Preamble\r\n", 17, 1000);
//					
//															
//					if((ManageFlag & 0x02) == 0x02)//检测到lora前导码结尾
//						HAL_UART_Transmit(&huart2, (uint8_t*)"Detect Preamble End\r\n", 21, 1000);
//						
//					
//					if((ManageFlag & 0x03) == 0x03)//检测到有效报头
//						HAL_UART_Transmit(&huart2, (uint8_t*)"Detect Head Is VALID\r\n", 22, 1000);						
					
					if((irqFlags & RFLR_IRQFLAGS_VALIDHEADER_MASK) == RFLR_IRQFLAGS_VALIDHEADER_MASK)
					{
					//	HAL_UART_Transmit(&huart2, (uint8_t*)"HeadISVALID\r\n", 13, 1000);
						
						SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER_MASK );
                    ...
                    {
                      //  HAL_UART_Transmit(&huart2, (uint8_t*)"CRC_ERROR\r\n", 11, 1000);
